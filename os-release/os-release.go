...
// Create OsRelease by parsing a file
func ParseFile(file io.Reader) (OsRelease, error) {
	osRelease := OsRelease{}
	...
	for scanner.Scan() {
		...
		if !found {
			return OsRelease{}, ErrCannotParseFile
		}
		switch key {
		case "ID":
...
func Parse() (OsRelease, error) {
	...
	if err != nil {
		...
		if err != nil {
			return OsRelease{}, errors.Join(ErrNoOsReleaseFile, err)
		}
	}
	defer f.Close()
	return ParseFile(f)
